...
(
	`order_id` INT,
    `product_id` INT,
    KEY pk_order_id_product_id(order_id, product_id),
...
# 05. Categories
-- resets the entire DATABASE online_store
...
# 09. Best categories
SELECT COUNT(c.name) AS items_count, c.name, SUM(p.quantity_in_stock) AS total_quantity
...
DETERMINISTIC
	BEGIN
		RETURN (SELECT COUNT(c.first_name) AS total_products
		FROM customers AS c
		JOIN orders AS o ON c.id = o.customer_id
